        ...
        public override void Run(int index, IReadOnlyDictionary<string, object> designVaribles)
        {
            ...
            if (pars?.Count > 0)
            {
                ...
                //检查所有文件是否都已经存在  然后进行相替换操作
                {
                    ...
                    int ycount = 0;
                    int zcount = 0;
                    var gslib = Gslib.ReadOut(faciemodel, out xcount, out ycount, out zcount);
                    ...
                    Gslib.WriteOut(Path.Combine(Main.GetWorkPath(), $"{index}", OutFileName), res, xcount, ycount, zcount);
                }
            }
            else
            {
